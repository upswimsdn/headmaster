...
public class StudentResourceImpl extends AbstractResource implements StudentResource {
     ...
     */
    @Override
    public List<Student> getStudents(String query, Boolean active, Boolean transferStudent, ClassYear classYear,
        ...
        validate((classYear == null && expectedGraduationYearFrom == null && expectedGraduationYearTo == null)
                || (classYear != null && expectedGraduationYearFrom == null && expectedGraduationYearTo == null)
                || (classYear == null && (expectedGraduationYearFrom != null || expectedGraduationYearTo != null)),
        ...
        if (isCumulativeGpaQuery || isTermGpaQuery) {
            ...
            validate((isCumulativeGpaQuery != isTermGpaQuery), Response.Status.BAD_REQUEST, ARGUMENT_CONFLICT);

            // Validate we don't have parameters from the two types of GPA queries mixing with each other
            validate(!(isCumulativeGpaQuery && gpaTermOrYearAreProvided), Response.Status.BAD_REQUEST,
                    ARGUMENT_CONFLICT);
            // Validate we don't have incomplete term gpa query
            validate(!(isTermGpaQuery && !gpaTermAndYearAreProvided), Response.Status.BAD_REQUEST, ARGUMENT_CONFLICT);
        } else {
        ...
        validate(query != null || classYear != null || expectedGraduationYearFrom != null
                || expectedGraduationYearTo != null || transferStudent != null || isCumulativeGpaQuery
                || isTermGpaQuery, Response.Status.BAD_REQUEST, QUERY_REQUIRED);

        // The classYear parameter produces an expected graduation year.
        if (classYear != null) {
            expectedGraduationYearFrom = expectedGraduationYearTo = classYear.getExpectedGraduationYear(new DateTime());
        }

        return studentService.getStudents(query != null ? preprocessQuery(query, skip, max, 0, 100) : null, active,
                transferStudent, expectedGraduationYearFrom, expectedGraduationYearTo, minCumulativeGpa,
                maxCumulativeGpa, minTermGpa, maxTermGpa, term, year, courseId, skip, max);
    }
     ...
     * 
     * @param student
     *            the student object to create. The student must have a null id.
    ...
    /**
     * Supposed to save the representation of the student with the given id. Inconsistent data should result in HTTP
     * 400, while a successful PUT should return Response.noContent.
     * 
     * @param id
     *            the id of the student to save.
     * @return A response with HTTP 204 no content on success, or a response with HTTP 400 and message
     *         <code>student.inconsistent</code> if checked data does not have the save id as requested in the URL.
     */
    @Override
     ...
     *             if there is no student with the given id, causing the framework to generate an HTTP 404.
     */
    @Override
